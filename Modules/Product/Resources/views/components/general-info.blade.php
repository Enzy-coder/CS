    ...
    <div class="dashboard__card__body custom__form general-info-form">
        <form action="#">
            <div class="row g-3 mt-2">
                ...
                @php 
                    $product_country_id = $product->country_id ?? '---';
                @endphp
                <div class="col-sm-12">
                    <div class="dashboard-input">
                        <label class="dashboard-label color-light mb-2"> {{ __('Product Country') }} </label>
                        <select name="country_id" class="form-control">
                            <option value="" disabled selected>Select a country</option>
                            @foreach($countries as $country)
                                <option {{$product_country_id == $country->id ? 'selected' : ''}} value="{{ $country->id }}">{{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
